...
class AppController extends Controller {
	...
	public $components = array(
	    'Auth' => array(
	        ...
	        'loginAction'=>array('controller'=>'users', 'action'=>'accueil')
	    ),
	    'Session'
	);
	...
	public function beforeFilter() {
		$this->Auth->allow( 'accueil', 'E001' , 'e013', 'valideMail');
		$this->Session->read(); 
		/**
		...
		if (isset($_SESSION['Dossier']['Eleve']['InterlocuteurId']) && isset($_SESSION['Dossier']['Parent']['ParentId']) ) {
			...
			$bon = $this->Fourniture->query(
				"SELECT * FROM bal_vue_e300_bon WHERE ConseilFCPEId=$conseilfcpe AND Exercice=$exercice AND InterlocuteurId =$interlocuteurid;"
			);
			for ($i=0; $i < sizeof($bon); $i++) {
				$bonoperation[$i][$bon[$i]['bal_vue_e300_bon']['BonOperationId']] = $bon[$i]['bal_vue_e300_bon']['TypeDeBonNom'];
			}
			
			if (!empty($bonoperation)){

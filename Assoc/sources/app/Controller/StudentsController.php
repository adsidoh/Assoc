	...
	class StudentsController extends AppController
	{
			...
			*/
			if($this->request->is('post')){
				...
				*/
				...
				// Interlocuteur id de l'eleve
				$interlocuteurId = $this->Student->query(
				" SELECT SUIVIIDLastId  FROM bal_suiviid WHERE
					SUIVIIDSessionId = '".$_SESSION['Auth']['User']['SessionId']."' AND SUIVIIDTableNom = 'BAL_Interlocuteur' 
					ORDER BY SUIVIIDId DESC LIMIT 0,1"
				);
				$this->Session->write('Dossier.Eleve.InterlocuteurId',$interlocuteurId[0]['bal_suiviid']['SUIVIIDLastId'] );
		...
		*/
		...
		public function E200Del(){
			...
			//Envoie  des informations sur l'eleve a la vue e200_del
		...
		public function deleteStudent(){
			...
			$eleve['EleveSessionId'] = $_SESSION['Auth']['User']['SessionId'];
     		$eleve['EleveId'] = $_SESSION['Dossier']['Eleve']['EleveId'];
	      	$this->makeCall("Delete_BAL_Vue_E200_Eleve", $eleve); // Supprime l'eleve de la base de données
			unset($_SESSION['Dossier']['Eleve']);  // Detruit dossier de l'eleve
		...
		*/
		public function E201(){
			...
			*/
			...
			$classe = $this->Student->query(
				"SELECT ClasseId,ClasseDesignation,Fin FROM bal_vue_e201_classe WHERE EtablissementId=".$_SESSION['Association']['EtablissementId']
			);

			$res = $this->Student->query(
				"SELECT * FROM bal_personne INNER JOIN bal_estresponsablelegal ON bal_estresponsablelegal.PersonneId = bal_personne.PersonneId WHERE bal_estresponsablelegal.EleveId =".$eleve['Student']['EleveId']
			);
			...
			*/
			if($this->request->is("post") && $this->request->data['classe'] != 0 ){
				...
				*/
				...
				$infoStudent = array(
					'PSessionId' => $_SESSION['Auth']['User']['SessionId'],
					'PEleveId'   => $_SESSION['Dossier']['Eleve']['EleveId'],
					'PExercice'  => $_SESSION['Exercice']
				);
				$this->makeCall('Compute_BAL_Vue_E300_PreCalcul' , $infoStudent );

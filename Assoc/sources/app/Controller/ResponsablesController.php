    ...
    */
    public function e100new(){
      ...
      if ($this->request->is('post')) {
        ...
        $nouveauParentId = $this->Responsable->query("SELECT
          SUIVIIDLastId FROM bal_suiviid WHERE SUIVIIDSessionId = '$user' AND SUIVIIDTableNom = 'BAL_Personne' ORDER BY SUIVIIDId DESC LIMIT 0,1;"
        );
        //recuperation de l'interlocuteurId du parent crée
        $nouveauParentInterlocuteurId = $this->Responsable->query(
          "SELECT SUIVIIDLastId FROM bal_suiviid WHERE SUIVIIDSessionId = '$user' AND SUIVIIDTableNom = 'BAL_Interlocuteur' ORDER BY SUIVIIDId DESC LIMIT 0,1;"
        );
